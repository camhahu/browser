...
export const registerNetworkCommands: RegisterCommand = (program) => {
    program
        ...
        .option(
            "-t, --type <types>",
            "Filter by type (comma-separated: xhr,fetch,document,script,stylesheet,image,font,websocket,other)",
        )
        ...
        .action(async (id, options) => {
            ...
            if (id) {
                ...
                for (const req of requests) {
                    ...
                    console.log(
                        `#${req.id.toString().padEnd(4)} ${method} ${status.padEnd(3)} ${url}  ${duration}${failed}`,
                    );
                }
            }
        });

    program.command("_network-daemon", { hidden: true }).action(runDaemon);
};
